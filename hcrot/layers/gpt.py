...
class GPTEmbedding(Module):
    def __init__(self, vocab_size: int, embed_dim: int, max_len: int):
        super().__init__()
        # These layers should be initialized with CuPy arrays for weights
        # if IS_CUDA is True, enabling GPU computation.
    ...
    def forward(self, x: NDArray) -> NDArray:
        ...
        # All operations on 'x' and its derivatives will now leverage CuPy
        # if 'x' is a CuPy array on the GPU.
        tok = self.token_embed(x) # (bsz, seq, embed_dim)

        # np.arange creates a CuPy array on the GPU if np is cupy
    ...
    def backward(self, dz: NDArray) -> Dict[str, NDArray]:
        dw = {}
        # The backward pass for Embedding layers should also handle CuPy arrays
        _, token_embed_dw = self.token_embed.backward(dz)
        dw["token_embed.weight"] = token_embed_dw

        # np.sum operates on the GPU if dz is a CuPy array
...
class FeedForward(Module):
    def __init__(self, embed_dim: int):
        super().__init__()
        # Linear layers and activations should be CuPy-compatible.
        self.fc = Linear(embed_dim, 4 * embed_dim)
        self.actv = GELU()
        self.fc2 = Linear(4 * embed_dim, embed_dim)

    def forward(self, x: NDArray) -> NDArray:
        # All sequential operations will execute on the GPU
        # if the input 'x' is a CuPy array.
        x = self.fc(x)
        x = self.actv(x)
        x = self.fc2(x)
        return x
    ...
    def backward(self, dz: NDArray) -> Tuple[NDArray, Dict[str, NDArray], Dict[str, NDArray]]:
        dw, db = {}, {}
        # Backward passes for Linear and GELU should also work with CuPy arrays.
...
class GPTBlock(Module):
    def __init__(self, embed_dim: int, num_heads: int):
        super().__init__()
        # LayerNorm and MultiHeadAttention should also be CuPy-compatible
    ...
    def forward(self, x: NDArray) -> NDArray:
        ...
        # Create a causal attention mask on the GPU using CuPy functions.
        # np.ones and np.triu will operate on the GPU.
        # Corrected mask creation from np.ones(seq) to np.ones((seq, seq))
        # for a 2D attention mask, and added dtype for clarity.
        mask = np.triu(np.ones((seq, seq), dtype=np.bool_), k=1)

        attn_output = self.attn(norm_x, norm_x, norm_x, attn_mask=mask) # self-attention
        x = x + attn_output # Element-wise addition on GPU
        x = x + self.ff(self.ln2(x)) # Sequential operations on GPU
        return x
    ...
    def backward(self, dz: NDArray) -> Tuple[NDArray, Dict[str, NDArray], Dict[str, NDArray]]:
        ...
        dz = dx_ + dz # Element-wise addition on GPU

        (dx_q, dx_k, dx_v), attn_dw, attn_db = self.attn.backward(dz)
        dx_ = dx_q + dx_k + dx_v # Element-wise addition on GPU
        ...
        dx = dx_ + dz # Element-wise addition on GPU
        return dx, dw, db
